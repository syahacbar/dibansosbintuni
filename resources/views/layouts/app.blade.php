...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full bg-slate-50">
    ...
    <body class="h-full font-sans antialiased text-slate-900 bg-slate-50 selection:bg-emerald-500 selection:text-white">
        ...
        <div x-data="{ sidebarOpen: false }" class="min-h-screen bg-slate-50">
            <!-- Sidebar Navigation -->
            <aside
                class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full overflow-y-auto border-r border-slate-800/80 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-300 transition-transform duration-300 ease-in-out lg:translate-x-0 shadow-2xl"
                :class="{ 'translate-x-0': sidebarOpen }"
            >
                <!-- Brand Header -->
                <div class="sticky top-0 z-10 flex h-20 items-center border-b border-slate-800/80 bg-slate-950/90 px-6 backdrop-blur-md">
                    <a href="{{ route('dashboard') }}" class="group flex min-w-0 items-center gap-3.5 transition-all">
                        @if ($logoPath)
                            <img src="{{ Storage::disk('public')->url($logoPath) }}" alt="Logo" class="h-10 w-10 shrink-0 rounded-xl border border-slate-700/60 bg-slate-900 p-1 object-contain shadow-md shadow-emerald-950/20 group-hover:scale-105 transition-transform">
                        @else
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 text-base font-bold text-white shadow-lg shadow-emerald-900/40 group-hover:scale-105 transition-transform">
                                DB
                            </div>
                        @endif
                        <div class="min-w-0">
                            <h2 class="truncate text-base font-bold tracking-tight text-white group-hover:text-emerald-400 transition-colors">
                                DIBANSOS BINTUNI
                            </h2>
            ...
            <div class="flex flex-col min-h-screen lg:pl-72 transition-all">
                <!-- Top Navbar -->
                <nav class="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur-md shadow-xs">
                    <div class="flex h-20 items-center justify-between px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="hidden md:flex flex-col items-end">
                                ...
                                <span class="inline-flex items-center gap-1.5 mt-0.5 rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-700 border border-emerald-200/80">
                            ...
                            <x-dropdown align="right" width="56">
                                <x-slot name="trigger">
                                    <button class="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 text-sm font-bold text-white shadow-md shadow-emerald-950/20 hover:scale-105 transition-transform focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
                                        {{ Str::of(Auth::user()->name)->substr(0, 1)->upper() }}
                                        <span class="absolute bottom-0 right-0 h-3 w-3 rounded-full bg-emerald-400 border-2 border-white"></span>
                                    </button>
                                </x-slot>
                ...
                <footer class="mt-auto border-t border-slate-200/80 bg-white px-6 py-4">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-slate-900">DIBANSOS BINTUNI</span>
                            <span>&bull;</span>
